...
def get_or_create_session(quiz_id):
    with _sessions_lock:
...
def calculate_leaderboard(quiz_id):
    with _sessions_lock:
...
def process_live_login(payload, client_socket):
...
def process_join_quiz(payload):
    ...
    session = get_or_create_session(quiz_id)
...
def process_answer(payload):
    ...
    leaderboard = calculate_leaderboard(quiz_id)
    broadcast_to_admins(
        {
            "type": "event",
